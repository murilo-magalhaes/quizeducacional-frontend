...
const GameCard: React.FC<GameCardProps> = ({game, onStartGame, onViewDetails}) => {
    // Estatísticas dos jogadores
    const playersInProgress = game.players.filter(p => p.gameStatus === EGameStatus.IN_PROGRESS);
    const playersCompleted = game.players.filter(p => p.gameStatus === EGameStatus.WON);
    const playersPaused = game.players.filter(p => p.gameStatus === EGameStatus.PAUSED);
    ...
    const cardFooter = (
        <div className="flex gap-2 py-3">
            <Button
                label={game.players.find(p => p.playerId === player.id)?.gameStatus === EGameStatus.IN_PROGRESS ? 'Continuar' : 'Iniciar'}
                icon="pi pi-play"
                className="flex-1"
                severity="success"
                ...
                <div className="grid">
                    <div className="col-6">
                        <div className="text-center">
                            <div className="text-2xl font-bold text-primary">{game.qntQuestions}</div>
                            <div className="text-sm text-600">Questões</div>
                        </div>
                    </div>
                    <div className="col-6">
                        <div className="text-center">
                            <div className="text-2xl font-bold text-orange-500">{totalPlayers}</div>
                    ...
                    <>
                        ...
                        <div className="mb-3">
                            <div className="flex align-items-center gap-2 mb-2">
                                ...
                                <span className="font-semibold text-600">Status dos Jogadores</span>
                            </div>
